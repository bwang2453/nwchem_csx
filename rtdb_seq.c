/* $Id: rtdb_seq.c 26754 2015-01-26 05:03:55Z jhammond $ */
#define USE_HDBM
#include <stdlib.h>
#include <sys/types.h>
...
*/
{
  ...
  if (nelem != nelem_from_db ) {
    (void) fprintf(stderr, 
		   "rtdb_seq_get: size mismatch \"%s\" in %s: arg=%d, db=%d\n",
		   name, rtdb[handle].filename, nelem, nelem_from_db);
    return 0;
  }
