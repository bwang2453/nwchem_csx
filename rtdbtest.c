/*$Id: rtdbtest.c 19708 2010-10-29 18:04:21Z d3y133 $*/
#include <stdio.h>
#include "rtdb.h"
#include "misc.h"
#include "macdecls.h"
...
int main(int argc, char *argv[])
{
  ...
  if (!rtdb_seq_open("h2o.db", "old", &rtdb))
    error("rtdbtest: open failed on %s\n", "test.db");

  /* Check character */

  if (!rtdb_seq_put(rtdb, "hello", MT_CHAR, 4, "bob"))
    error("rtdbtest: c put failed\n", 0);

  if (!rtdb_seq_get(rtdb, "hello", MT_CHAR, 4, buf))
  ...
  ibuf = 1;
  if (!rtdb_seq_put(rtdb, "ibuf", MT_INT, 1, &ibuf))
    error("rtdbtest: i put failed\n", 0);

  ibuf = 0;
  if (!rtdb_seq_get(rtdb, "ibuf", MT_INT, 1, &ibuf))
    error("rtdbtest: i get failed\n", 0);
//  if (!rtdb_seq_get(rtdb, "geometry:geometry:ncenter", MT_INT, 1, &ibuf))
//    error("rtdbtest: i get failed\n", 0);
  ...
  dbuf = 1.0;
  if (!rtdb_seq_put(rtdb, "dbuf", MT_DBL, 1, &dbuf))
    error("rtdbtest: d put failed\n", 0);

  dbuf = 0.0;
  if (!rtdb_seq_get(rtdb, "dbuf", MT_DBL, 1, &dbuf))
...

  {
    ...
    while (n--) {
      ...
      if (!rtdb_seq_get_info(rtdb, names[n], &ma_type, &nelem, date))
  ...
  
  {
    ...
    for (available=rtdb_seq_first(rtdb, sizeof name, name);
	 available;
	 available=rtdb_seq_next(rtdb, sizeof name, name)) {
  ...
  rtdb_seq_print(rtdb, 1);
  
//  if (!rtdb_seq_close(rtdb, "delete"))
